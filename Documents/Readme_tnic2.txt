****************************
TOP LEVEL NOTE: This is a bigger version of the standard TNIC-3 database, which is the default that folks use in the literature.  This version has more than twice the peers deemed as being "in the classification" as 4.5% of randomly drawn firms are deemed to be peers according to this classification (TNIC-2).  This classification is called "TNIC-2" because it is calibrated to be as granular as two digit SIC codes.  By comparison, the baseline TNIC-3 database is calibrated to be as granular as three-digit SIC codes.
...
This file accompanies the TNIC-2 industry databases and describes where the data comes from,
...
The TNIC-2 classification data we are distributing only records firms having pairwise similarities with a given firm i that are 
above a threshold as required based on the coarseness of the two digit SIC classification.  The level of coarseness of TNIC-2 thus matches 
that of two digit SIC codes, as both classifications result in the same number of firm pairs being deemed related.  For example, if one picks 
two firms at random from the CRSP/COMPUSTAT universe, the likelihood of them being in the same two digit SIC code is about 4.5%.  Analgously, 
when the TNIC-2 cutoff is specified using our approach, the likelihood of two randomly drawn firms being deemed related in their TNIC-2 is 
also 4.5%.  Hence, TNIC-2 is constructed to be "as coarse" as are two digit SIC codes.
